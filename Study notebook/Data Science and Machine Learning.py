...
# ДРОПНУВШИЙСЯ тот кто не дошел до конца и не появлялся больше месяца
# мы знаем, что финальный день это 2018/05/19 (timestamp = 1526772811)
# а месяц пропуска это таймстемп = 2592000
# Посчитаем максимальное появление время на курсе, максимальный таймстемп
print(events_data.groupby('user_id', as_index=False).agg({'timestamp': 'max'}).head())

# сколько времени прошло с последнего момента захода на текущий день (2018/05/19)
# Заведем датафрейм в котором будем все хранить
users_data = events_data.groupby('user_id', as_index=False).agg({'timestamp': 'max'})

# переименовали, можно в одну строчку
users_data = users_data.rename(columns={'timestamp': 'last_timestamp'})
# заводим новую колонку в которую записываем разницу сколько прошло времени с последнего захода юзера до сегодняшнего дня
users_data['is_gone_user'] = 1526772811 - users_data.last_timestamp

now_day = events_data.timestamp.max()  # 1526772811            # завели переменные в которой храним последний день
drop_out_treshold = 2592000     # и время после которого считается пропуск, сейчас это месяц (30 * 24 * 60 * 60)

users_data['is_gone_user'] = (now_day - users_data.last_timestamp) > drop_out_treshold
# получается наш пользователь будет записан в гон юзер, так как он отсутствовал на платформе месяц (тру)

# Стоит в таблицу добавить по количеству попыток
# Сначала добавим правильные и не правильные попытки
# У нас есть users_scores с количеством попыток, но просто с merge мы потеряли данные, и пользователей без попыток (напр 1) потерялся
# по стандарту merge делает пересечением
# (users_data = users_data.merge(users_scores)), проверяет какие есть в обоих и совмещает, тех что нет, просто сливает
# Надо добавить how='outer' (users_data = users_data.merge(users_scores, how='outer')) и тогда появятся NaN
# и что бы самому определить по какой колонке он будет merge, надо добавить on='user_id'
# users_data = users_data.merge(users_scores, on='user_id', how='outer')

users_data = users_data.merge(users_scores, how='outer')
# NaN заменим на нули 0
users_data = users_data.fillna(0)
# Получится, что в is_gone_user показатель False мы ждем, что вернется, а True уже больше месяца нетя
# users_data = users_data.merge(users_scores, on='user_id', how='outer').fillna(0)
# так нельзя в одну строчку, разные данные будут


# для кажого user есть подсчет событий
...
# Добавим к нашему датайфрейму, еще событий
users_data = users_data.merge(users_events_data, on='user_id', how='outer')

# Получится общий датафрейм в котором будет user_id - пользователь / last_timestamp - последнее появление /
# is_gone_user - считается дропнутым / correct and wrong - правильные и неправильные сабмиты /
# discovered - число степов которые открыл / passed - выполненные степы / started_attempt - начинал проходить /
# viewed - посмотрел

# Не хватает, сколько уникальных дней пользователь был в степике, получится серия в которой мы взяли и создали группу по юзеру и уникальные дни
# users_days = events_data.groupby('user_id').day.nunique()
# print(users_days.head())
# print(users_days.to_frame())    # что бы в датафрейм превратить, но индексы поплыли
users_days = events_data.groupby('user_id').day.nunique().to_frame().reset_index()
# получится датафрейм не серия, с нормальными индексами и теперь добавим в общий датафрейм в котором все собираем
users_data = users_data.merge(users_days, how='outer')

# print(users_data.user_id.nunique())     # проверяем не потеряли строки
# 175 пройденных шагов - значит успешно прошел курс, создадим колонку с этим показателем
# просто возьмем юзер дату, его количество прошедшиг степов
users_data['passed_course'] = users_data.passed > 170
print(users_data)
print(users_data.groupby('passed_course').count())    # вывод количества студентов которые прошли все или нет
print(users_data['passed_course'].value_counts(normalize=True))  # вывод количества % студентов которые прошли все или нет
...
data = pd.DataFrame({
    ...
    'X_2': [0, 0, 0, 1, 0, 0, 0, 1],
    'Y': [1, 1, 1, 1, 0, 0, 0, 0]
})      # маленький и легкий датафрейм

# научиться при помощи дерева решений предсказывать значения переменной Y,
# основываясь на значениях переменной X_1 и X_2
# Перед тем, как дерево решений обучить чему-либо, надо его сначала посадить.

clf = tree.DecisionTreeClassifier()

# дерево решений - это объект, имеющий свои методы и атрибуты. Так же, как например и пандасовский датафрейм.
# Методы обучиться / предсказать что-нибудь и далее
clf = tree.DecisionTreeClassifier(criterion='entropy')
X = data[['X_1', 'X_2']]  # все фичи в переменную X (те данные, которые мы используем для обучения)
y = data.Y  # сохраним в переменную y и пусть это будет пандасовская серия (то, что мы предсказываем)

clf.fit(X, y)
# первое обучение дерева метод fit() и передаем в него переменную с фичами и переменную с целевой переменной
...
pets_1 = pets.drop('Unnamed: 0', axis=1)  # удалили не нужный столбец, что бы в графике не было Unnamed, а было гавкает
